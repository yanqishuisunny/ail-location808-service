...
public class UserRealm extends AuthorizingRealm {
     ...
     */
    @Override
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws BusinessException, AuthenticationException {
        String token = (String) auth.getCredentials();
        if (Strings.isNullOrEmpty(token)) {
            log.warn("token丢失");
            throw new BusinessException(BusCode.TOKEN_LOSE);
        }
        try {
            Claims claims = jwtUtil.getClaimFromToken(token);
            String userId = claims.getSubject();
            String redisKey = CachePre.LOING_SHIRO_JWT_ID + userId;
            log.info("userId : {}" , userId);
            long expire = RedisUtil.getExpire(redisKey);
            if(!RedisUtil.hasKey(redisKey) && expire <= 0){
                throw new BusinessException(BusCode.TOKEN_EXPIRATION);
            }

            String redisToken = (String) RedisUtil.get(redisKey);
            if(!Objects.equals(token,redisToken)){
                throw new BusinessException(BusCode.OTHER_LOGIN_ERROR);
            }
            ...
            throw new BusinessException(BusCode.TOKEN_INVALID);
        }
    }
}
